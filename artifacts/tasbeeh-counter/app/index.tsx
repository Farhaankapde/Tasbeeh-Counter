...
export default function HomeScreen() {
  ...
  const queuePersist = (state: AppState) => {
    return storagePersister.enqueue(state)
      .then(() => {
        if (mountedRef.current) setPersistenceWarning(null);
      })
      .catch((error: unknown) => {
        if (__DEV__) console.error('Tasbeeh persistence write failed:', error);
        if (mountedRef.current) setPersistenceWarning(PERSISTENCE_WARNING);
      });
  };
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      try {
        ...
        if (active) {
          setPersistenceWarning('Unable to load saved progress. Your current session is safe.');
          setHydrated(true);
        }
        return;
      }
      if (!active) return;
      try {
        const storedValue = raw === null ? null : JSON.parse(raw) as unknown;
        const migrated = restoreStoredState(storedValue, DEFAULT_STATE, LEGACY_DHIKRS);
        appStateRef.current = migrated;
        setAppState(migrated);
      } catch (error: unknown) {
        if (__DEV__) console.error('Tasbeeh stored state is malformed:', error);
        setPersistenceWarning('Unable to read saved progress. Your current session is safe; the saved copy was left unchanged.');
      } finally {
  ...
  const prefetchThemeAssets = async (theme: AccentTheme) => {
    const sources = [COUNTER_IMAGES[theme], DIAL_IMAGES[theme], theme === 'arabesque-white' ? ARABESQUE_BACKGROUND : CINEMATIC_BACKGROUND];
    const uris = sources
      .map((source) => {
        try {
    ...
    await Promise.all(uris.map(async (uri) => {
      if (prefetchedAssetUris.current.has(uri)) return;
      const loaded = await Image.prefetch(uri).catch(() => false);
      if (loaded) prefetchedAssetUris.current.add(uri);
    }));
  };
  ...
  useEffect(() => {
    const inactiveThemes = (Object.keys(COUNTER_IMAGES) as AccentTheme[]).filter((theme) => theme !== appState.accentTheme);
    void Promise.all(inactiveThemes.map((theme) => prefetchThemeAssets(theme)));
  }, [appState.accentTheme]);
  ...
  const changeSetting = <K extends keyof AppState>(key: K, value: AppState[K]) => {
    if (!hydrated) return;
    updateAppState((previous) => ({ ...previous, [key]: value }));
  };
  const resetCurrent = () => {
    if (!hydrated) return;
    if (anonymousCounter) {
  ...
  const chooseDhikr = (id: string) => {
    if (!hydrated) return;
    activeHistorySessionId.current = null;
    updateAppState((previous) => ({ ...previous, selectedId: id }));
  ...
  const confirmDeleteDhikr = () => {
    if (!hydrated || !deleteDhikrId) return;
    ...
    <LinearGradient colors={isSandalwood || isArabesqueWhite ? [palette.background, palette.card, palette.background] : appState.theme === 'light' ? [palette.background, '#e9e4de', palette.background] : [palette.background, '#171211', palette.background]} start={{ x: 0.1, y: 0 }} end={{ x: 0.9, y: 1 }} style={styles.root}>
      <StatusBar barStyle={appState.theme === 'light' ? 'dark-content' : 'light-content'} />
       {activeTab === 'counter' ? <View style={[styles.counterScroll, { overflow: 'hidden', paddingHorizontal: 21, paddingTop: counterSafeTop + 8, paddingBottom: 80 + Math.max(insets.bottom, 14) }]}>
        <CounterAtmosphere dark={appState.theme !== 'light'} accent={palette.primaryBright} sandalwood={isSandalwood} arabesqueWhite={isArabesqueWhite} />
         <View style={[styles.counterLayer, { flex: 1, minHeight: 0 }]}>
           ...
           <View style={[styles.counterDeviceStage, compactCounter && styles.counterDeviceStageCompact, { flex: 1 }]}>{counterAssetsReady ? <HardwareCounter count={currentCount} width={deviceWidth} palette={palette} counterImage={COUNTER_IMAGES[appState.accentTheme]} counterTheme={appState.accentTheme} scale={scale} pressed={pressed} completionFlash={completionFlash} disabled={!hydrated} onPress={increment} onPressIn={() => setPressed(true)} onPressOut={() => setPressed(false)} /> : <View style={[styles.hardware, { width: deviceWidth, height: deviceWidth * 1.38, backgroundColor: palette.surfaceStrong, opacity: 0.35 }]} />}</View>
          <View style={[styles.counterQuote, compactCounter && styles.counterQuoteCompact, { marginBottom: 50, backgroundColor: palette.card, borderColor: palette.border, borderWidth: 1, borderRadius: 24, shadowColor: palette.shadow, shadowOpacity: 0.2, shadowRadius: 14, shadowOffset: { width: 0, height: 5 }, elevation: 3 }]}><Text style={[styles.counterQuoteText, { color: palette.foreground }]}>“In the remembrance of Allah{'\n'}do hearts find peace.”</Text><View style={[styles.counterQuoteRule, { backgroundColor: palette.primaryBright }]} /><Text style={[styles.counterQuoteCitation, { color: palette.muted }]}>(Quran 13:28)</Text></View>
              <View pointerEvents="box-none" style={{ alignItems: 'flex-end', marginTop: compactCounter ? 0 : 3, paddingRight: 3, position: 'absolute', right: 0, bottom: 4, zIndex: 10, elevation: 10 }}><Pressable testID="reset-counter" accessibilityRole="button" accessibilityLabel={'Reset ' + selectedDhikr.name + ' counter'} accessibilityState={{ disabled: !hydrated }} disabled={!hydrated} hitSlop={10} onPressIn={() => { if (appStateRef.current.vibration) Haptics.selectionAsync().catch(() => undefined); }} onPress={() => setResetting(true)} style={({ pressed: p }) => [{ width: 46, height: 46, borderRadius: 23, borderWidth: 1, borderColor: isSandalwood || isArabesqueWhite ? palette.primary : palette.border, backgroundColor: 'rgba(18, 18, 18, 0.68)', alignItems: 'center', justifyContent: 'center', shadowColor: palette.primary, shadowOpacity: 0.18, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 3, transform: [{ scale: p ? 0.9 : 1 }], opacity: p ? 0.78 : 1 }]}><Feather name="rotate-ccw" size={18} color={isSandalwood || isArabesqueWhite ? palette.destructive : palette.primaryBright} /></Pressable></View>
         </View>
              </View> : <SecondaryTab tab={activeTab} palette={palette} appState={appState} todayCount={todayCount} weekCount={weekCount} totalCount={totalCount} historyGroupKey={historyGroupKey} stateReady={hydrated} onHistoryGroupChange={setHistoryGroupKey} onChooseDhikr={chooseDhikr} onAddDhikr={() => openDhikrEditor()} onEditDhikr={openDhikrEditor} onDeleteDhikr={setDeleteDhikrId} />}
       {persistenceWarning ? <View accessibilityRole="alert" accessibilityLiveRegion="polite" style={[styles.persistenceNotice, { backgroundColor: palette.card, borderColor: palette.border }]}><Feather name="alert-circle" size={15} color={palette.primaryBright} /><Text style={[styles.persistenceNoticeText, { color: palette.foreground }]}>{persistenceWarning}</Text></View> : null}
       ...
       <Modal visible={deleteDhikrId !== null} transparent animationType="fade" onRequestClose={() => setDeleteDhikrId(null)}><View style={styles.confirmRoot}><Pressable style={styles.modalBackdrop} onPress={() => setDeleteDhikrId(null)} /><View style={[styles.confirmCard, { backgroundColor: palette.card, borderColor: palette.border }]}><View style={[styles.confirmIcon, { backgroundColor: palette.destructive }]}><Feather name="trash-2" size={22} color={palette.primaryForeground} /></View><Text style={[styles.confirmTitle, { color: palette.foreground }]}>Delete Dhikr?</Text><Text style={[styles.confirmBody, { color: palette.muted }]}>Its current count and target will be removed. History and lifetime totals stay safe.</Text><View style={styles.confirmActions}><Pressable accessibilityRole="button" accessibilityLabel="Cancel delete" onPress={() => setDeleteDhikrId(null)} style={[styles.confirmButton, { backgroundColor: palette.surface }]}><Text style={[styles.confirmButtonText, { color: palette.foreground }]}>Cancel</Text></Pressable><Pressable testID="confirm-delete-dhikr" accessibilityRole="button" accessibilityLabel="Confirm delete Dhikr" onPress={confirmDeleteDhikr} style={[styles.confirmButton, { backgroundColor: palette.destructive }]}><Text style={[styles.confirmButtonText, { color: palette.primaryForeground }]}>Delete</Text></Pressable></View></View></View></Modal>
         ...
         <SettingsModal visible={settingsOpen} stateReady={hydrated} appState={appState} palette={palette} topInset={insets.top} bottomInset={insets.bottom} onClose={() => setSettingsOpen(false)} onChange={changeSetting} />
...
function SecondaryTab({ tab, palette, appState, todayCount, weekCount, totalCount, historyGroupKey, stateReady, onHistoryGroupChange, onChooseDhikr, onAddDhikr, onEditDhikr, onDeleteDhikr }: { tab: Tab; palette: Palette; appState: AppState; todayCount: number; weekCount: number; totalCount: number; historyGroupKey: string | null; stateReady: boolean; onHistoryGroupChange: (key: string | null) => void; onChooseDhikr: (id: string) => void; onAddDhikr: () => void; onEditDhikr: (item: DhikrRecord) => void; onDeleteDhikr: (id: string) => void }) {
  const insets = useSafeAreaInsets();
    ...
    <ScrollView style={{ zIndex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={[styles.secondaryContent, { paddingTop: secondarySafeTop + 18, paddingBottom: 110 + Math.max(insets.bottom, 14) }]}>
      <View style={[styles.secondaryHeader, { borderBottomWidth: 1, borderBottomColor: palette.border, paddingBottom: 16, marginBottom: 20 }]}>
       ...
       {tab === 'history' ? <HistorySection appState={appState} palette={palette} selectedGroupKey={historyGroupKey} onSelectedGroupKeyChange={onHistoryGroupChange} />
        ...
        : <View>
            <Pressable testID="add-dhikr" accessibilityRole="button" accessibilityLabel="Add Dhikr" accessibilityState={{ disabled: !stateReady }} disabled={!stateReady} onPress={onAddDhikr} style={({ pressed }) => [styles.plusButton, styles.premiumPrimaryButton, { alignSelf: 'flex-end', backgroundColor: palette.primary, borderColor: palette.primaryBright, width: 158, height: 50, borderRadius: 28, marginBottom: 16, shadowColor: palette.primary, shadowOpacity: 0.42, shadowRadius: 16, elevation: 8, opacity: stateReady ? 1 : 0.45 }, pressed && styles.pressed]}><Text style={[styles.plusText, { color: palette.primaryForeground, fontFamily: Platform.OS === 'android' ? 'serif' : 'Georgia', fontSize: 16, lineHeight: 20, textAlign: 'center' }]}>Add Dhikr</Text></Pressable>
           {appState.dhikrs.length === 0 ? <EmptyPanel icon="bookmark" title="Your Dhikr Library is Empty" body="Add your first Dhikr to begin counting." palette={palette} /> : appState.dhikrs.map((item) => <View key={item.id} testID={`dhikr-row-${item.id}`} style={[styles.libraryRow, styles.premiumLibraryRow, { minHeight: 92, borderRadius: 22, paddingHorizontal: 13, backgroundColor: appState.theme === 'light' ? palette.card : appState.accentTheme === 'sandalwood' ? 'rgba(42, 27, 19, 0.92)' : appState.accentTheme === 'arabesque-white' ? 'rgba(255, 253, 247, 0.9)' : 'rgba(25, 22, 21, 0.9)', borderColor: palette.border, shadowColor: palette.primary, shadowOpacity: 0.16, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 6 }]}>
             <Pressable accessibilityRole="button" accessibilityLabel={`Select ${item.name}`} accessibilityState={{ disabled: !stateReady }} disabled={!stateReady} onPress={() => onChooseDhikr(item.id)} style={[styles.librarySelect, { minHeight: 78, opacity: stateReady ? 1 : 0.45 }]}>
              <DhikrMark palette={palette} size={56} />
               ...
               <View style={[styles.libraryMeta, { alignItems: 'center', justifyContent: 'center', gap: 8, paddingLeft: 5 }]}><Feather name="chevron-right" size={23} color={palette.foreground} /><View style={[styles.libraryActions, { gap: 6 }]}><Pressable testID={`edit-dhikr-${item.id}`} accessibilityRole="button" accessibilityLabel={`Edit ${item.name}`} accessibilityState={{ disabled: !stateReady }} disabled={!stateReady} onPress={() => onEditDhikr(item)} style={({ pressed }) => [styles.libraryAction, { backgroundColor: palette.surface, borderColor: palette.border, opacity: !stateReady ? 0.45 : pressed ? 0.68 : 1 }]}><Feather name="edit-2" size={16} color={palette.primaryBright} /></Pressable><Pressable testID={`delete-dhikr-${item.id}`} accessibilityRole="button" accessibilityLabel={`Delete ${item.name}`} accessibilityState={{ disabled: !stateReady }} disabled={!stateReady} onPress={() => onDeleteDhikr(item.id)} style={({ pressed }) => [styles.libraryAction, { backgroundColor: appState.theme === 'light' ? 'rgba(177, 35, 46, 0.08)' : 'rgba(255, 92, 103, 0.12)', borderColor: appState.theme === 'light' ? 'rgba(177, 35, 46, 0.26)' : 'rgba(255, 110, 119, 0.36)', opacity: !stateReady ? 0.45 : pressed ? 0.68 : 1 }]}><Feather name="trash-2" size={16} color={palette.destructive} /></Pressable></View></View>
          </View>)}
        </View>}
    </ScrollView>
  </View>;
      ...
      <View style={themeStyles.settingsLayer}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.settingsScroll}>
          <SettingsSection title="COUNTER" palette={palette}>
             <SettingRow icon="vibrate" label="Vibration" value={appState.vibration} onValueChange={(value) => onChange('vibration', value)} palette={palette} disabled={!stateReady} />
            <SettingDivider palette={palette} />
             <SettingRow icon="volume-2" label="Sound" value={appState.sound} onValueChange={(value) => onChange('sound', value)} palette={palette} feather disabled={!stateReady} />
            <SettingDivider palette={palette} />
             <SettingRow icon="activity" label="Counter animation" value={appState.counterAnimation} onValueChange={(value) => onChange('counterAnimation', value)} palette={palette} feather disabled={!stateReady} />
            <SettingDivider palette={palette} />
             <SettingRow icon="flag" label="Stop counting at target" value={appState.stopAtTarget} onValueChange={(value) => onChange('stopAtTarget', value)} palette={palette} feather disabled={!stateReady} />
          </SettingsSection>
          <SettingsSection title="APPEARANCE" palette={palette}>
             <AccentThemePicker value={appState.accentTheme} onChange={(theme) => onChange('accentTheme', theme)} palette={palette} disabled={!stateReady} />
            <SettingDivider palette={palette} />
             <View style={styles.themeRow}><View style={[styles.settingIcon, { backgroundColor: palette.primary }]}><Feather name="sun" size={17} color={palette.primaryForeground} /></View><Text style={[styles.settingLabel, { color: palette.foreground }]}>Mode</Text><View style={[styles.segmented, { backgroundColor: palette.surface }]}>{(['dark', 'light'] as const).map((theme) => <Pressable key={theme} testID={`display-mode-${theme}`} accessibilityRole="button" accessibilityLabel={`${theme === 'dark' ? 'Dark' : 'Light'} mode`} accessibilityState={{ selected: appState.theme === theme, disabled: !stateReady }} disabled={!stateReady} onPress={() => onChange('theme', theme)} style={[styles.segment, { opacity: stateReady ? 1 : 0.45 }, appState.theme === theme && { backgroundColor: palette.primary }]}><Feather name={theme === 'dark' ? 'moon' : 'sun'} size={14} color={appState.theme === theme ? palette.primaryForeground : palette.muted} /><Text style={[styles.segmentText, { color: appState.theme === theme ? palette.primaryForeground : palette.muted }]}>{theme === 'dark' ? 'Dark' : 'Light'}</Text></Pressable>)}</View></View>
          </SettingsSection>
...
function SettingRow({ icon, label, value, onValueChange, palette, feather = false, disabled = false }: { icon: string; label: string; value: boolean; onValueChange: (value: boolean) => void; palette: Palette; feather?: boolean; disabled?: boolean }) { return <View style={[styles.settingRow, disabled && { opacity: 0.45 }]}><View style={[styles.settingIcon, { backgroundColor: palette.primary }]}>{feather ? <Feather name={icon as keyof typeof Feather.glyphMap} size={17} color={palette.primaryForeground} /> : <MaterialCommunityIcons name={icon as keyof typeof MaterialCommunityIcons.glyphMap} size={18} color={palette.primaryForeground} />}</View><Text style={[styles.settingLabel, { color: palette.foreground }]}>{label}</Text><Switch testID={'toggle-' + label.toLowerCase().replace(' ', '-')} accessibilityLabel={'Toggle ' + label} accessibilityState={{ disabled }} disabled={disabled} value={value} onValueChange={onValueChange} trackColor={{ false: palette.surfaceStrong, true: palette.greenSoft }} thumbColor={value ? palette.primaryBright : palette.muted} ios_backgroundColor={palette.surfaceStrong} /></View>; }
function AccentThemePicker({ value, onChange, palette, disabled = false }: { value: AccentTheme; onChange: (theme: AccentTheme) => void; palette: Palette; disabled?: boolean }) {
  const options: Array<{ id: AccentTheme; label: string; color: string; image?: number }> = [
    ...
    return <View style={[{ paddingVertical: 14 }, disabled && { opacity: 0.45 }]}>
