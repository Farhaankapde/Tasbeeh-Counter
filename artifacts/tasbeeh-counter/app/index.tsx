...
export default function HomeScreen() {
  ...
  const counterSafeTop = Platform.OS === 'ios' ? Math.max(insets.top, 52) : Platform.OS === 'android' ? Math.max(insets.top, 24) : Math.max(insets.top, 18);
      ...
      {activeTab === 'counter' ? appState.dhikrs.length > 0 ? <View style={[styles.counterScroll, { overflow: 'hidden', paddingHorizontal: 21, paddingTop: counterSafeTop + 8, paddingBottom: 80 + Math.max(insets.bottom, 14) }]}>
        <CounterAtmosphere dark={appState.theme !== 'light'} accent={palette.primaryBright} />
         ...
         <View style={[styles.counterQuote, compactCounter && styles.counterQuoteCompact]}><Text style={[styles.counterQuoteText, { color: palette.foreground }]}>“In the remembrance of Allah{'\n'}do hearts find peace.”</Text><View style={[styles.counterQuoteRule, { backgroundColor: palette.primaryBright }]} /><Text style={[styles.counterQuoteCitation, { color: palette.muted }]}>(Quran 13:28)</Text></View>
          <View style={{ alignItems: 'flex-end', marginTop: compactCounter ? 0 : 3, paddingRight: 3 }}><Pressable testID="reset-counter" accessibilityRole="button" accessibilityLabel={'Reset ' + selectedDhikr.name + ' counter'} hitSlop={10} onPressIn={() => { void Haptics.selectionAsync(); }} onPress={() => setResetting(true)} style={({ pressed: p }) => [{ width: 46, height: 46, borderRadius: 23, borderWidth: 1, borderColor: palette.border, backgroundColor: 'rgba(18, 18, 18, 0.68)', alignItems: 'center', justifyContent: 'center', shadowColor: palette.primary, shadowOpacity: 0.18, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 3, transform: [{ scale: p ? 0.9 : 1 }], opacity: p ? 0.78 : 1 }]}><Feather name="rotate-ccw" size={18} color={palette.primaryBright} /></Pressable></View>
         </View>
...
function CounterAtmosphere({ dark, accent }: { dark: boolean; accent: string }) {
  return <View pointerEvents="none" style={styles.counterAtmosphere}>
    ...
    <View style={{ position: 'absolute', width: 320, height: 420, borderRadius: 160, top: 165, alignSelf: 'center', backgroundColor: accent, opacity: dark ? 0.08 : 0.045, shadowColor: accent, shadowOpacity: 0.48, shadowRadius: 50, elevation: 2 }} />
...
function HistorySection({ appState, palette }: { appState: AppState; palette: Palette }) {
  ...
  return <View>{sectionOrder.map((section) => {
    ...
    return <View key={section} style={styles.historyDateSection}><Text style={[styles.historyDateLabel, { color: palette.primaryBright }]}>{section}</Text>{sectionGroups.map((group) => {
      ...
      return <Pressable key={group.key} accessibilityRole="button" accessibilityLabel={`View ${group.dhikr} sessions`} onPress={() => setSelectedGroupKey(group.key)} style={({ pressed }) => [styles.historyGroupRow, styles.premiumHistoryGroup, { backgroundColor: palette.card, borderColor: palette.border, opacity: pressed ? 0.78 : 1 }]}><DhikrMark palette={palette} size={38} /><View style={styles.historyGroupCopy}><Text style={[styles.historyName, { color: palette.foreground }]}>{group.dhikr}</Text><Text style={[styles.historySummary, { color: palette.primaryBright }]}>{total.toLocaleString()} repetitions</Text><Text style={[styles.historyLatest, { color: palette.muted }]}>{dateNote ? `${dateNote} • ` : ''}{sessionNote}</Text></View><Feather name="chevron-right" size={18} color={palette.muted} /></Pressable>;
    })}</View>;
  })}</View>;
}
...
function AccentThemePicker({ value, onChange, palette }: { value: AccentTheme; onChange: (theme: AccentTheme) => void; palette: Palette }) {
  const options: Array<{ id: AccentTheme; label: string; color: string }> = [
  ...
  return <View style={{ paddingVertical: 14 }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }}><View style={[styles.settingIcon, { backgroundColor: palette.primary }]}><Feather name="droplet" size={17} color={palette.primaryForeground} /></View><View><Text style={[styles.settingLabel, { color: palette.foreground }]}>Color theme</Text><Text style={[styles.settingHint, { color: palette.muted }]}>Choose your accent and glow</Text></View></View>
    <View style={{ flexDirection: 'row', gap: 7 }}>{options.map((option) => { const selected = value === option.id; return <Pressable key={option.id} testID={`accent-theme-${option.id}`} accessibilityRole="button" accessibilityLabel={`Use ${option.label} theme`} accessibilityState={{ selected }} onPress={() => onChange(option.id)} style={({ pressed }) => [{ flex: 1, minHeight: 68, borderRadius: 15, borderWidth: 1, borderColor: selected ? option.color : palette.border, backgroundColor: palette.surface, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4, opacity: pressed ? 0.72 : 1 }, selected && { shadowColor: option.color, shadowOpacity: 0.28, shadowRadius: 10, elevation: 4 }]}><View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: option.color, borderWidth: selected ? 2 : 0, borderColor: palette.primaryForeground, alignItems: 'center', justifyContent: 'center' }}>{selected ? <Feather name="check" size={12} color={palette.primaryForeground} /> : null}</View><Text style={{ color: selected ? palette.foreground : palette.muted, fontFamily: 'Inter_500Medium', fontSize: 10, marginTop: 7, textAlign: 'center' }}>{option.label}</Text></Pressable>; })}</View>
  </View>;
}
