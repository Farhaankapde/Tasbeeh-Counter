...
export default function HomeScreen() {
  ...
  return (
    <LinearGradient colors={appState.theme === 'light' ? [palette.background, '#e9e4de', palette.background] : [palette.background, '#171211', palette.background]} start={{ x: 0.1, y: 0 }} end={{ x: 0.9, y: 1 }} style={styles.root}>
      ...
      {activeTab === 'counter' ? appState.dhikrs.length > 0 ? <ScrollView style={styles.counterScroll} showsVerticalScrollIndicator={false} bounces={false} contentContainerStyle={[styles.scrollContent, styles.counterScrollContent, { paddingTop: insets.top + 16, paddingBottom: 104 + Math.max(insets.bottom, 14) }]}>
        <CounterAtmosphere palette={palette} dark={appState.theme !== 'light'} />
        <View style={styles.counterLayer}>
          <View style={[styles.topBar, styles.referenceTopBar]}><IconButton icon="menu" label="Open menu" onPress={() => setMenuOpen(true)} palette={palette} /><View style={styles.counterHeaderCopy}><Text style={[styles.counterTitle, { color: palette.foreground }]}>Tasbeeh Counter</Text><Text style={[styles.counterSubtitle, { color: palette.muted }]}>Remember. Reflect. Repeat.</Text></View><IconButton icon="settings" label="Open settings" onPress={() => setSettingsOpen(true)} palette={palette} /></View>
         <View style={[styles.selector, styles.referenceSelector, styles.premiumSelector, { backgroundColor: appState.theme === 'light' ? palette.card : 'rgba(30, 30, 30, 0.9)', borderColor: completionFlash ? palette.primaryBright : palette.border }]}>
          <Pressable testID="dhikr-selector" accessibilityRole="button" accessibilityLabel={'Select Dhikr, currently ' + selectedDhikr.name} onPress={() => setSelectorOpen(true)} style={({ pressed: selectorPressed }) => [styles.selectorMain, { opacity: selectorPressed ? 0.82 : 1 }]}>
             <View style={[styles.selectorIcon, styles.referenceSelectorIcon, { backgroundColor: palette.primary }]}><MaterialCommunityIcons name={selectedDhikr.icon as keyof typeof MaterialCommunityIcons.glyphMap} size={24} color={palette.primaryForeground} /></View>
         ...
         <View style={styles.counterDeviceStage}><HardwareCounter count={currentCount} width={deviceWidth} palette={palette} scale={scale} pressed={pressed} completionFlash={completionFlash} disabled={!hydrated} onPress={increment} onPressIn={() => setPressed(true)} onPressOut={() => setPressed(false)} /><View style={[styles.counterQuote, { borderBottomColor: palette.primaryBright }]}><Text style={[styles.counterQuoteText, { color: palette.muted }]}>Small{'\n'}Dhikrs{'\n'}Make a{'\n'}Big Difference</Text></View></View>
         <View style={styles.referenceActions}><Pressable testID="reset-counter" accessibilityRole="button" accessibilityLabel={'Reset ' + selectedDhikr.name + ' counter'} onPress={() => setResetting(true)} style={({ pressed: p }) => [styles.referenceAction, styles.resetAction, { borderColor: palette.border, backgroundColor: appState.theme === 'light' ? palette.card : 'rgba(20, 20, 20, 0.82)' }, p && styles.pressed]}><Feather name="rotate-ccw" size={21} color={palette.foreground} /><Text style={[styles.referenceActionText, { color: palette.foreground }]}>Reset</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Save count now" onPress={saveNow} style={({ pressed: p }) => [styles.referenceAction, styles.saveAction, { backgroundColor: savedFlash ? palette.primaryBright : palette.primary, borderColor: palette.primaryBright }, p && styles.pressed]}><Feather name={savedFlash ? 'check' : 'save'} size={21} color={palette.primaryForeground} /><Text style={[styles.referenceActionText, { color: palette.primaryForeground }]}>{savedFlash ? 'Saved' : 'Save'}</Text></Pressable></View>
        </View>
...
function CounterAtmosphere({ palette, dark }: { palette: Palette; dark: boolean }) {
  return <View pointerEvents="none" style={styles.counterAtmosphere}>
    <LinearGradient colors={dark ? ['#171311', '#070707', '#12100d'] : ['#f1ece7', '#d8d0c8', '#efe9e4']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
    <View style={[styles.atmosphereGlow, { backgroundColor: dark ? 'rgba(132, 22, 19, 0.22)' : 'rgba(183, 31, 42, 0.08)' }]} />
    <View style={[styles.textileFold, { borderColor: dark ? 'rgba(190, 150, 112, 0.08)' : 'rgba(80, 58, 44, 0.08)' }]} />
    <View style={[styles.textileFold, styles.textileFoldLower, { borderColor: dark ? 'rgba(190, 150, 112, 0.1)' : 'rgba(80, 58, 44, 0.1)' }]} />
    <View style={styles.patternAccent}><MaterialCommunityIcons name="star-four-points" size={18} color={dark ? 'rgba(194, 150, 104, 0.22)' : 'rgba(115, 79, 46, 0.14)'} /><MaterialCommunityIcons name="star-four-points" size={9} color={dark ? 'rgba(194, 150, 104, 0.16)' : 'rgba(115, 79, 46, 0.1)'} /></View>
    <View style={styles.beadCluster}>{[0, 1, 2, 3, 4, 5, 6].map((bead) => <View key={bead} style={[styles.bead, { backgroundColor: dark ? 'rgba(93, 45, 32, 0.62)' : 'rgba(125, 74, 56, 0.2)', transform: [{ translateX: bead * 15 }, { translateY: Math.sin(bead * 0.8) * 10 }] }]} />)}</View>
    <View style={[styles.atmosphereVignette, { borderColor: dark ? 'rgba(0, 0, 0, 0.7)' : 'rgba(55, 39, 30, 0.08)' }]} />
    <View style={[styles.atmosphereRule, { backgroundColor: palette.primaryBright }]} />
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  counterScroll: { flex: 1 }, counterScrollContent: { flexGrow: 1 }, counterAtmosphere: { ...StyleSheet.absoluteFill, overflow: 'hidden' }, counterLayer: { zIndex: 1 }, referenceTopBar: { minHeight: 66 }, counterHeaderCopy: { alignItems: 'center', flex: 1 }, counterTitle: { fontFamily: Platform.OS === 'android' ? 'serif' : 'Georgia', fontSize: 26, letterSpacing: -0.4 }, counterSubtitle: { fontFamily: Platform.OS === 'android' ? 'serif' : 'Georgia', fontSize: 13, marginTop: 2, letterSpacing: 0.1 }, referenceSelector: { minHeight: 88, borderRadius: 27, marginTop: 10, marginBottom: 14, paddingHorizontal: 13 }, referenceSelectorIcon: { width: 54, height: 54, borderRadius: 27 }, counterDeviceStage: { minHeight: 460, alignItems: 'center', justifyContent: 'center', position: 'relative', marginTop: 1 }, counterQuote: { position: 'absolute', right: 0, bottom: 20, paddingBottom: 7, paddingLeft: 8, borderBottomWidth: 1 }, counterQuoteText: { fontFamily: Platform.OS === 'android' ? 'serif' : 'Georgia', fontStyle: 'italic', fontSize: 12, lineHeight: 16, textAlign: 'right' }, referenceActions: { flexDirection: 'row', gap: 10, marginTop: 5 }, referenceAction: { flex: 1, minHeight: 58, borderRadius: 30, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, shadowColor: '#000', shadowOpacity: 0.28, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 6 }, resetAction: {}, saveAction: {}, referenceActionText: { fontFamily: Platform.OS === 'android' ? 'serif' : 'Georgia', fontSize: 17, fontWeight: '600' }, atmosphereGlow: { position: 'absolute', width: 280, height: 280, borderRadius: 140, top: 170, alignSelf: 'center' }, textileFold: { position: 'absolute', width: '150%', height: 220, left: '-25%', top: 430, borderTopWidth: 1, borderRadius: 140, transform: [{ rotate: '-10deg' }] }, textileFoldLower: { top: 650, transform: [{ rotate: '8deg' }] }, patternAccent: { position: 'absolute', left: 17, top: 285, transform: [{ rotate: '-18deg' }], gap: 7 }, beadCluster: { position: 'absolute', right: -23, top: 475, width: 150, height: 36, transform: [{ rotate: '34deg' }] }, bead: { position: 'absolute', width: 22, height: 22, borderRadius: 11, borderWidth: 1, borderColor: 'rgba(226, 156, 107, 0.16)' }, atmosphereVignette: { ...StyleSheet.absoluteFill, borderWidth: 22, borderRadius: 32 }, atmosphereRule: { position: 'absolute', width: 32, height: 2, bottom: 72, right: 0, borderRadius: 1 },
});
