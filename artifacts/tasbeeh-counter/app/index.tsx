        ...
        : <View>
           ...
           {appState.dhikrs.length === 0 ? <EmptyPanel icon="bookmark" title="Your Dhikr Library is Empty" body="Add your first Dhikr to begin counting." palette={palette} /> : appState.dhikrs.map((item) => <View key={item.id} testID={`dhikr-row-${item.id}`} style={[styles.libraryRow, styles.premiumLibraryRow, { minHeight: 92, borderRadius: 22, paddingHorizontal: 13, backgroundColor: appState.theme === 'light' ? palette.card : appState.accentTheme === 'sandalwood' ? 'rgba(42, 27, 19, 0.92)' : appState.accentTheme === 'arabesque-white' ? 'rgba(255, 253, 247, 0.9)' : 'rgba(25, 22, 21, 0.9)', borderColor: palette.border, shadowColor: palette.primary, shadowOpacity: 0.16, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 6 }]}>
            <Pressable accessibilityRole="button" accessibilityLabel={`Select ${item.name}`} onPress={() => onChooseDhikr(item.id)} style={[styles.librarySelect, { minHeight: 78 }]}>
              ...
              <View style={[styles.libraryMeta, { alignItems: 'center', justifyContent: 'center', gap: 8, paddingLeft: 5 }]}><Feather name="chevron-right" size={23} color={palette.foreground} /><View style={[styles.libraryActions, { gap: 6 }]}><Pressable testID={`edit-dhikr-${item.id}`} accessibilityRole="button" accessibilityLabel={`Edit ${item.name}`} onPress={() => onEditDhikr(item)} style={({ pressed }) => [styles.libraryAction, { backgroundColor: palette.surface, borderColor: palette.border, opacity: pressed ? 0.68 : 1 }]}><Feather name="edit-2" size={16} color={palette.primaryBright} /></Pressable><Pressable testID={`delete-dhikr-${item.id}`} accessibilityRole="button" accessibilityLabel={`Delete ${item.name}`} onPress={() => onDeleteDhikr(item.id)} style={({ pressed }) => [styles.libraryAction, { backgroundColor: appState.theme === 'light' ? 'rgba(177, 35, 46, 0.08)' : 'rgba(255, 92, 103, 0.12)', borderColor: appState.theme === 'light' ? 'rgba(177, 35, 46, 0.26)' : 'rgba(255, 110, 119, 0.36)', opacity: pressed ? 0.68 : 1 }]}><Feather name="trash-2" size={16} color={palette.destructive} /></Pressable></View></View>
          </View>)}
        </View>}
    </ScrollView>
  </View>;
