...
export default function HomeScreen() {
    ...
    const hydrate = async () => {
      try {
        ...
        if (raw && active) {
          ...
          setAppState({ ...DEFAULT_STATE, ...parsed, counters, history: parsed.history ?? [], theme: parsed.theme === 'light' ? 'light' : 'dark' });
        }
      } catch {
  ...
  const increment = () => {
    ...
    setAppState((previous) => {
      ...
      const nextHistory = last && last.dhikr === selectedDhikr.name
        ? [{ ...last, repetitions: last.repetitions + 1, time }, ...previous.history.slice(1)]
        : [{ id: String(Date.now()), dhikr: selectedDhikr.name, repetitions: 1, time }, ...previous.history].slice(0, 30);
      return { ...previous, counters: { ...previous.counters, [selectedDhikr.id]: nextCount }, history: nextHistory };
    });
...
function HardwareCounter({ count, width, palette, scale, pressed, onPress, onPressIn, onPressOut }: { count: number; width: number; palette: Palette; scale: Animated.Value; pressed: boolean; onPress: () => void; onPressIn: () => void; onPressOut: () => void }) {
  ...
  return <View style={[styles.hardware, { width, height: width * 1.38, shadowColor: palette.shadow }]}>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="Reset counter" onPress={() => Alert.alert('Reset Counter?', 'Use the reset control below to confirm.')} style={styles.resetHitArea} />
  </View>;
}
...
function SecondaryTab({ tab, palette, appState, totalCount, onChooseDhikr }: { tab: Tab; palette: Palette; appState: AppState; totalCount: number; onChooseDhikr: (id: string) => void }) {
  const insets = useSafeAreaInsets();
  const title = tab === 'history' ? 'History' : tab === 'stats' ? 'Statistics' : 'Dhikr Library';
  return <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.secondaryContent, { paddingTop: insets.top + 18, paddingBottom: 110 + Math.max(insets.bottom, 14) }]}><View style={styles.secondaryHeader}><Text style={[styles.secondaryTitle, { color: palette.foreground }]}>{title}</Text><Text style={[styles.secondarySubtitle, { color: palette.muted }]}>{tab === 'history' ? 'Your recent remembrance sessions' : tab === 'stats' ? 'A quiet view of your progress' : 'Choose a remembrance to continue'}</Text></View>{tab === 'history' ? <View>{appState.history.length === 0 ? <EmptyPanel icon="clock" title="No sessions yet" body="Your completed counting sessions will appear here." palette={palette} /> : appState.history.map((entry) => <View key={entry.id} style={[styles.historyRow, { backgroundColor: palette.card, borderColor: palette.border }]}><View style={[styles.historyIcon, { backgroundColor: palette.primary }]}><MaterialCommunityIcons name="counter" size={18} color={palette.primaryForeground} /></View><View style={styles.historyCopy}><Text style={[styles.historyName, { color: palette.foreground }]}>{entry.dhikr}</Text><Text style={[styles.historyTime, { color: palette.muted }]}>{entry.time}</Text></View><Text style={[styles.historyCount, { color: palette.primaryBright }]}>+{entry.repetitions}</Text></View>)}</View> : tab === 'stats' ? <View><View style={styles.statsGrid}><MetricCard icon="calendar-today" label="Today" value={appState.counters[appState.selectedId] ?? 0} palette={palette} /><MetricCard icon="calendar-week" label="This Week" value={totalCount} palette={palette} /><MetricCard icon="chart-bar" label="Total" value={totalCount} palette={palette} /></View><View style={[styles.goalCard, { backgroundColor: palette.card, borderColor: palette.border }]}><View style={[styles.goalIcon, { borderColor: palette.primary }]}><MaterialCommunityIcons name="target" size={19} color={palette.primaryBright} /></View><View style={styles.goalCopy}><Text style={[styles.goalLabel, { color: palette.foreground }]}>Daily Goal</Text><Text style={[styles.goalValue, { color: palette.muted }]}>{(appState.counters[appState.selectedId] ?? 0).toLocaleString()} / 1,000</Text></View><View style={styles.goalProgressWrap}><View style={[styles.goalTrack, { backgroundColor: palette.surfaceStrong }]}><View style={[styles.goalProgress, { width: Math.min((appState.counters[appState.selectedId] ?? 0) / 1000, 1) * 100, backgroundColor: palette.primary }]} /></View></View><Text style={[styles.goalPercent, { color: palette.primaryBright }]}>{Math.round(Math.min((appState.counters[appState.selectedId] ?? 0) / 1000, 1) * 100)}%</Text></View><View style={[styles.longPanel, { backgroundColor: palette.card, borderColor: palette.border }]}><Text style={[styles.panelEyebrow, { color: palette.primaryBright }]}>CURRENT PRACTICE</Text><Text style={[styles.panelTitle, { color: palette.foreground }]}>{totalCount.toLocaleString()} repetitions</Text><Text style={[styles.panelBody, { color: palette.muted }]}>Keep returning to the count that matters today.</Text></View></View> : <View>{DEFAULT_DHIKR.map((item) => <Pressable key={item.id} onPress={() => onChooseDhikr(item.id)} style={[styles.libraryRow, { backgroundColor: palette.card, borderColor: palette.border }]}><View style={[styles.historyIcon, { backgroundColor: palette.primary }]}><MaterialCommunityIcons name={item.icon} size={18} color={palette.primaryForeground} /></View><View style={styles.historyCopy}><Text style={[styles.historyName, { color: palette.foreground }]}>{item.name}</Text><Text style={[styles.historyTime, { color: palette.muted }]}>{item.arabic}</Text></View><Text style={[styles.historyCount, { color: palette.primaryBright }]}>{String(appState.counters[item.id] ?? 0).padStart(3, '0')}</Text></Pressable>)}</View>}</ScrollView>;
}
